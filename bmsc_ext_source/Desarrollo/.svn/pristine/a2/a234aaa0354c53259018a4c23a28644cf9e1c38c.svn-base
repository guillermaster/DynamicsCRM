/****** Object:  StoredProcedure [dbo].[sp_servicio_deudas_consumo]    Script Date: 08/27/2013 11:35:57 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE procedure [dbo].[sp_servicio_deudas_consumo] (@c_cliente varchar(max), @c_serviciodeuda money  output ) as
begin
...
declare c1 cursor for
    select c_tipo, c_descripcion, c_porcentaje , c_regla1 ,  c_regla2  from  @tabproceso where c_tipo <> 1;
open c1
	fetch next from c1
    ...
    deallocate c1;
     ...
     select @c_serviciodeuda =  (isnull(ac.efk_cuotas_BMSC,0)) + (isnull(ac.efk_cuotas_BMSC_tramite,0)) + sum(isnull(ac.efk_deuda_empresa_empleadora,0)) +  SUM(isnull(tp.c_regla2,0))
    from BMSC_MSCRM..Account  ac , @tabproceso tp
    where ac.AccountId = @c_cliente 
    group by ac.AccountId,ac.efk_cuotas_BMSC,ac.efk_cuotas_BMSC_tramite
