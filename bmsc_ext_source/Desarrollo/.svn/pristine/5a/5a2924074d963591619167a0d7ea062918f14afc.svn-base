...
{
    public partial class ReabrirOferta : System.Web.UI.Page
    {
        ...
        private void AbrirOferta(int numOferta)
        {
            try
            {
                ...
                if (!negOferta.ReabrirOfertaRiesgos(numOferta))
                {
                    Utilidades.Alert(this, "REABRIR_OFERTA_RIESGOS", negOferta.Error);
                }
                CerrarVentana(true);
            }
            catch (Exception ex)
            {
                Utilidades.ReportarError(this, Request.QueryString[Utilidades.NombresParametros.Usuario], "No se pudo reabrir oferta.", "REABRIR_OFERTA_RIESGOS", ex);
                CerrarVentana(false);
            }
        }
        ...
        private void CerrarVentana(bool exito)
        {
            ...
            this.ClientScript.RegisterStartupScript(this.GetType(), "RETORNAR_REABRIR_OFERTA", jscript1);
            string jscript2 = "<script type=\"text/javascript\"> window.close(); </script>";
            this.ClientScript.RegisterStartupScript(this.GetType(), "CERRAR_VENTANA_REABRIR_OFERTA", jscript2);
        }
    }
}