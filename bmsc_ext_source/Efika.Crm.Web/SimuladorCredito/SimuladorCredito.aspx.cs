    ...
    public partial class SimuladorCredito : System.Web.UI.Page
    {
        ...
        protected void InicializarSimulacionExistente()
        {
            try
            {
                ...
                cs.Attributes = new string[] { "efk_name", "efk_numero_oferta", "efk_camapaniaid", "efk_simulacion_guardada", "efk_divisaid" };
                ...
                CargarCodDivisa(simCred.DivisaId, false);
        ...
        public void CargarProductosSimulados()
        {
            if (hdnCargarProdsSims.Value == "1")//solo recargar prods.sims. si se ha indicado (hdnCargarProdsSims=1)
            {
                ...
                {
                    //si el botón de moneda está habilitado, significa que hay que guardar la moneda de la simulación
                    SetMonedaSimulacion();
        ...
        public void SetMonedaSimulacion()
        {
            if (btnMonedaLookup.Enabled && !string.IsNullOrWhiteSpace(hdnCodDivisa.Value))
            {
                try
                {
                    SimulacionCredito negSimCred = new SimulacionCredito(Credenciales.ObtenerCredenciales());
                    negSimCred.ActualizarMoneda(new Guid(hdnSimCredito.Value), new Guid(hdnCodDivisa.Value));
                }
                catch (Exception ex)
                {
                    Utilidades.ReportarError(this.Page, Request.QueryString[Utilidades.NombresParametros.Usuario], "No se pudo guardar la moneda de la simulación, intente nuevamente por favor.",
                        "SIMULACION_GUARDAR_MONEDA", ex);
                }
            }
        }
