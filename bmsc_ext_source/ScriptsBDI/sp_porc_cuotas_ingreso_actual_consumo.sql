...
declare @resultado money
 ...
 select @resultado
*/
CREATE  procedure  [dbo].[sp_porc_cuotas_ingreso_actual_consumo](@c_nrosolicitud int , @resultado money output, @resultado2 money output) as
begin
    ...
    exec sp_servicio_deudas_consumo @c_cliente, @servdeudas output 
 ...
 select pr.efk_productohomologado 
	...
	where op.efk_numero_oferta = @c_numeroferta and op.efk_nrosolicitud = @c_nrosolicitud
  OPEN @cursor1
  FETCH NEXT 
  ...
  DEALLOCATE @cursor1 
    ...
    select  @resultado2 =  dbo.fn_sp_porcrelac(@aux_prodhom,dbo.fn_sp_ingprod(@c_cliente))
