/****** Object:  StoredProcedure [dbo].[sp_servicio_deudas_fsc]    Script Date: 08/27/2013 11:35:57 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE procedure [dbo].[sp_servicio_deudas_fsc] (@nro_solicitud varchar(max), @c_serviciodeuda money  output ) as
begin
...
open c1
    ...
    begin
    		...
    		from BMSC_MSCRM..efk_foto_solicitud_credito 
			where efk_nrosolicitud = @nro_solicitud 
    ...
    deallocate c1;
     ...
     select @c_serviciodeuda = sum(isnull(f.efk_cuotas_BMSC,0)) + sum(isnull(f.efk_cuotas_BMSC_tramite,0)) + sum(isnull(f.efk_deuda_empresa_empleadora,0))  +  SUM(tp.total)
    from 
    BMSC_MSCRM..efk_foto_solicitud_credito  f , 
    (select SUM(isnull(c_regla2,0)) total from @tabproceso) tp
    where f.efk_nrosolicitud = @nro_solicitud 
